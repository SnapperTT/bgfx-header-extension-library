// tonemappingFilter.hh
//

#ifndef LZZ_tonemappingFilter_hh
#define LZZ_tonemappingFilter_hh
#include "bgfxh.h"
...
//
// How to use:
// 0. Include! Use #include <bgfxh/bgfxh.h>, #include <bgfxh/tonemappingFilter.h> in a relevent file. Be sure to use #define BGFXH_IMPL *once* to generate an implementation somewhere!
// 1. Create an instance of the tonemappingFilter object ( bgfxh::tonemappingFilter mTonemappingFilter
// 2. Set the user parameters to your liking. Use mTonemappingFilter.setSize(width, height);
// 3. Call tonemappingFilter.init() - this will generate framebuffers and uniforms and load the shaders
...
namespace bgfxh
{
  void tonemappingFilter::setSize (uint32_t const _width, uint32_t const _height)
                                                                     {
		width = _width;
		height = _height; 
		}
...
namespace bgfxh
{
  bgfx::FrameBufferHandle tonemappingFilter::getOutputFrameBuffer () const
                                                              { return mOutputFB; }
}
namespace bgfxh
{
  int tonemappingFilter::getViewIncrement () const
                                      { return 1; }
}
namespace bgfxh
{
  void tonemappingFilter::setExtraComponent (uint32_t slot, bgfx::TextureHandle const texture, float additiveWeight, float multiplicitveWeight)
                                                                                                                                   {
		BX_CHECK (index < 4, "Only 4 tonemapping attachments are supported!");
		BX_CHECK (index < maxAdditonalSamplerSlots, "tonemapping: maxAdditonalSamplerSlots must be set to the maximum number of channels you wish to use before you call init()!");
...
namespace bgfxh
{
  void tonemappingFilter::initToZero ()
                           {
...
namespace bgfxh
{
  void tonemappingFilter::init ()
		...
		for (unsigned int i = 0; i < 4; ++i) {
			...
			#include "shaders/tonemapping_filter/c/vs_tonemapping.bin.h"
			#include "shaders/tonemapping_filter/c/fs_tonemapping.bin.h"
			...
			#include "shaders/tonemapping_filter/c/vs_tonemapping_ch1.bin.h"
			#include "shaders/tonemapping_filter/c/fs_tonemapping_ch1.bin.h"
			...
			#include "shaders/tonemapping_filter/c/vs_tonemapping_ch2.bin.h"
			#include "shaders/tonemapping_filter/c/fs_tonemapping_ch2.bin.h"
			...
			#include "shaders/tonemapping_filter/c/vs_tonemapping_ch3.bin.h"
			#include "shaders/tonemapping_filter/c/fs_tonemapping_ch3.bin.h"
			...
			#include "shaders/tonemapping_filter/c/vs_tonemapping_ch4.bin.h"
			#include "shaders/tonemapping_filter/c/fs_tonemapping_ch4.bin.h"
		...
		#endif
		BX_CHECK(m_tonemappingProgram, "failed to load shader bgfxh::tonemappingFilter::m_tonemappingProgram! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
		inited = true;
		}
}
namespace bgfxh
{
  void tonemappingFilter::deInit ()
                       {
...
namespace bgfxh
{
  void tonemappingFilter::submit (bgfx::TextureHandle const colourBufferIn, bgfx::TextureHandle const lumBufferIn)
                                                                                                      {
