...
namespace bgfxh
{
  extern bgfx::ProgramHandle m_programUntexturedPassthroughColor;
}
namespace bgfxh
{
...
namespace bgfxh
{
  struct frustum
  {
    ...
    bool frustumCheck4 (bx::Vec3 const & where, float const objectBoundingRadius) const;
    bool frustumCheck4 (float const * bgfxModelMtx, float const objectBoundingRadius) const;
    bool frustumCheck5 (bx::Vec3 const & where, float const objectBoundingRadius) const;
    bool frustumCheck5 (float const * bgfxModelMtx, float const objectBoundingRadius) const;
    bool frustumCheck6 (bx::Vec3 const & where, float const objectBoundingRadius) const;
    bool frustumCheck6 (float const * bgfxModelMtx, float const objectBoundingRadius) const;
    void buildViewFrustum (float * out, float const fovX, float const fovY, float const zNear, float const zFar);
  };
}
namespace bgfxh
{
...
namespace bgfxh
{
  void debugDrawLines (bgfx::ViewId const viewId, bx::Vec3 const * segments, uint32_t const numSegments, uint32_t const colorRGBA, uint64_t state = 0);
}
namespace bgfxh
{
  template <typename T>
...
namespace bgfxh
{
  LZZ_INLINE bool frustum::frustumCheck4 (bx::Vec3 const & where, float const objectBoundingRadius) const
                                                                                                           {
...
namespace bgfxh
{
  LZZ_INLINE bool frustum::frustumCheck4 (float const * bgfxModelMtx, float const objectBoundingRadius) const
                                                                                                               {
			bx::Vec3 where(bgfxModelMtx[12], bgfxModelMtx[13], bgfxModelMtx[14]);
			return frustumCheck4(where, objectBoundingRadius);
			}
}
namespace bgfxh
{
  LZZ_INLINE bool frustum::frustumCheck5 (bx::Vec3 const & where, float const objectBoundingRadius) const
                                                                                                           {
			/// To do: sse optimisation
			/// Checks left/right/top/bottom/near
			return (dotPV(planes[0], where) + objectBoundingRadius> 0.0f) &&
					(dotPV(planes[1], where) + objectBoundingRadius> 0.0f) &&
					(dotPV(planes[2], where) + objectBoundingRadius> 0.0f) &&
					(dotPV(planes[3], where) + objectBoundingRadius> 0.0f) &&
					(dotPV(planes[4], where) + objectBoundingRadius> 0.0f);
			}
}
namespace bgfxh
{
  LZZ_INLINE bool frustum::frustumCheck5 (float const * bgfxModelMtx, float const objectBoundingRadius) const
                                                                                                               {
			bx::Vec3 where(bgfxModelMtx[12], bgfxModelMtx[13], bgfxModelMtx[14]);
			return frustumCheck5(where, objectBoundingRadius);
			}
}
namespace bgfxh
{
  LZZ_INLINE bool frustum::frustumCheck6 (bx::Vec3 const & where, float const objectBoundingRadius) const
                                                                                                           {
			/// To do: sse optimisation
			/// Checks left/right/top/bottom/near/far
			return (dotPV(planes[0], where) + objectBoundingRadius> 0.0f) &&
					(dotPV(planes[1], where) + objectBoundingRadius> 0.0f) &&
					(dotPV(planes[2], where) + objectBoundingRadius> 0.0f) &&
					(dotPV(planes[3], where) + objectBoundingRadius> 0.0f) &&
					(dotPV(planes[4], where) + objectBoundingRadius> 0.0f) &&
					(dotPV(planes[5], where) + objectBoundingRadius> 0.0f);
			}
}
namespace bgfxh
{
  LZZ_INLINE bool frustum::frustumCheck6 (float const * bgfxModelMtx, float const objectBoundingRadius) const
                                                                                                               {
			bx::Vec3 where(bgfxModelMtx[12], bgfxModelMtx[13], bgfxModelMtx[14]);
			return frustumCheck6(where, objectBoundingRadius);
			}
}
namespace bgfxh
{
