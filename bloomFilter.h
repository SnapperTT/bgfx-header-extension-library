...
#ifndef LZZ_bloomFilter_hh
#define LZZ_bloomFilter_hh
#include "bgfxh.h"
...
// 
// How to use:
// 0. Include! Use #include <bgfxh/bgfxh.h>, #include <bgfxh/bloomFilter.h> in a relevent file. Be sure to use #define BGFXH_IMPL *once* to generate an implementation somewhere!
// 1. Create an instance of the bloomFilter object ( bgfxh::bloomFilter mBloomFilter
...
namespace bgfxh
{
  bgfx::FrameBufferHandle bloomFilter::getOutputFrameBuffer () const
                                                              {
...
namespace bgfxh
{
  int bloomFilter::getViewIncrement () const
                                       {
...
namespace bgfxh
{
  void bloomFilter::setSize (uint32_t const _bloomWidth, uint32_t const _bloomHeight)
                                                                               {
...
namespace bgfxh
{
  void bloomFilter::init ()
		...
		u_screenSize = bgfx::createUniform("u_screenSize", bgfx::UniformType::Vec4);
			...
			if (useLum) {
			#include "shaders/bloom_filter/c/vs_bloom_brightpass_lum.bin.h"
			#include "shaders/bloom_filter/c/fs_bloom_brightpass_lum.bin.h"
			...
			#include "shaders/bloom_filter/c/vs_bloom_brightpass.bin.h"
			#include "shaders/bloom_filter/c/fs_bloom_brightpass.bin.h"
			...
			{
			#include "shaders/bloom_filter/c/vs_bloom_blur_hz.bin.h"
			#include "shaders/bloom_filter/c/fs_bloom_blur_hz.bin.h"
			...
			{
			#include "shaders/bloom_filter/c/vs_bloom_blur_vt.bin.h"
			#include "shaders/bloom_filter/c/fs_bloom_blur_vt.bin.h"
		...
		BX_CHECK(m_bloomExtractProgram, "failed to load shader bgfxh::bloomFilter::m_bloomExtractProgram! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
		BX_CHECK(m_programBloomBlur[0], "failed to load shader bgfxh::bloomFilter::m_programBloomBlur[0]! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
		BX_CHECK(m_programBloomBlur[1], "failed to load shader bgfxh::bloomFilter::m_programBloomBlur[1]! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
...
namespace bgfxh
{
  void bloomFilter::submit_common (bgfx::TextureHandle const & colourBufferIn, bgfx::TextureHandle const & lumBufferIn)
                                                                                                                 {
...
namespace bgfxh
{
  void bloomFilter::submit (bgfx::TextureHandle const & colourBufferIn, bgfx::TextureHandle const & lumBufferIn)
                                                                                                          {
		BX_CHECK(useLum, "bgfxh::bloomFilter: submiting with lum buffer when configured to not use one! Set useLum to true before init()!");
		BX_CHECK(inited, "using a bgfxh::bloomFilter without calling init()!");
...
namespace bgfxh
{
  void bloomFilter::submit (bgfx::TextureHandle const & colourBufferIn)
                                                                 {
		BX_CHECK(!useLum, "bgfxh::bloomFilter: submiting without a lum buffer!");
		BX_CHECK(inited, "using a bgfxh::bloomFilter without calling init()!");
