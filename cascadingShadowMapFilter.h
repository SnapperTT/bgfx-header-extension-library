// cascadingShadowMapFilter.hh
//

#ifndef LZZ_cascadingShadowMapFilter_hh
#define LZZ_cascadingShadowMapFilter_hh
...
namespace bgfxh
{
  class cascadingShadowMapFilter : public filter
  {
  public:
...
namespace bgfxh
{
  LZZ_INLINE void cascadingShadowMapFilter::submit (uint32_t const level)
                                                  {
...
namespace bgfxh
{
  cascadingShadowMapFilter::cascadingShadowMapFilter ()
                                    { initToZero(); }
}
namespace bgfxh
{
...
namespace bgfxh
{
  bgfx::FrameBufferHandle cascadingShadowMapFilter::getOutputFrameBuffer (unsigned int const csmLevel) const
                                                                                         { return m_shadowMapFB[csmLevel]; }
}
namespace bgfxh
{
  bgfx::TextureHandle cascadingShadowMapFilter::getOutputTexture (unsigned int const csmLevel) const
...
namespace bgfxh
{
  bgfx::FrameBufferHandle cascadingShadowMapFilter::getOutputFrameBuffer () const
                                                              { return getOutputFrameBuffer(0); }
}
namespace bgfxh
{
  bgfx::TextureHandle cascadingShadowMapFilter::getOutputTexture () const
                                                      { return getOutputTexture(0); }
}
namespace bgfxh
{
  int cascadingShadowMapFilter::getViewIncrement () const
                                      { return nShadowLevels; }
}
namespace bgfxh
{
  void cascadingShadowMapFilter::initToZero ()
                           {
...
namespace bgfxh
{
  void cascadingShadowMapFilter::init ()
                     {
		...
		for (unsigned int i = 0; i < nShadowLevels; ++i) {
			...
			if (useVsm) {
			#include "shaders/cascading_shadow_map_filter/c/vs_cascading_shadow_map_vsm.bin.h"
			#include "shaders/cascading_shadow_map_filter/c/fs_cascading_shadow_map_vsm.bin.h"
			...
			#include "shaders/cascading_shadow_map_filter/c/vs_cascading_shadow_map.bin.h"
			#include "shaders/cascading_shadow_map_filter/c/fs_cascading_shadow_map.bin.h"
		...
		BX_CHECK(bgfx::isValid(m_programDepthWrite), "failed to load shader bgfxh::cascadingShadowMapFilter::m_programDepthWrite! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
		inited = true;
		}
}
namespace bgfxh
{
  void cascadingShadowMapFilter::deInit ()
                       {
		for (unsigned int i = 0; i < BGFXH_MAX_SHADOW_LEVELS; ++i) {
...
namespace bgfxh
{
  void cascadingShadowMapFilter::setupViews (float * viewMtx, float * projMtx, bx::Vec3 const & lightDirection, float const FOV_H_DEG, float const FOV_V_DEG)
                                                                                                                                          {
...
namespace bgfxh
{
  bool cascadingShadowMapFilter::isObjectInShadowVolume (bx::Vec3 const & _pos, float const radius, unsigned int const csmLevel)
                                                                                                             {
...
namespace bgfxh
{
  float cascadingShadowMapFilter::internal_sqDistPointAabb (bx::Vec3 const & point, bx::Vec3 const & aabbMin, bx::Vec3 const & aabbMax)
                                                                                                                    {
...
namespace bgfxh
{
  void cascadingShadowMapFilter::bindSamplers (unsigned int shadowSampler0)
                                                        {
...
namespace bgfxh
{
  void cascadingShadowMapFilter::setLightMVP (float const * modelMtx)
                                                  {
		for (unsigned int csmLevel = 0; csmLevel < nShadowLevels; ++csmLevel) {
			float mvp_shadow[16];
