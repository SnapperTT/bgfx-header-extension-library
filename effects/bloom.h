...
namespace bgfxh
{
  void bloomEffect::init ()
		...
		BGFXH_CHECK(bgfx::isValid(m_bloomExtractProgram), "failed to load shader bgfxh::bloomEffect::m_bloomExtractProgram! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
		BGFXH_CHECK(bgfx::isValid(m_programBloomBlur[0]), "failed to load shader bgfxh::bloomEffect::m_programBloomBlur[0]! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
		BGFXH_CHECK(bgfx::isValid(m_programBloomBlur[1]), "failed to load shader bgfxh::bloomEffect::m_programBloomBlur[1]! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
...
namespace bgfxh
{
  void bloomEffect::submit (bgfx::TextureHandle const & colourBufferIn, bgfx::TextureHandle const & lumBufferIn)
                                                                                                          {
		BGFXH_CHECK(useLum, "bgfxh::bloomEffect: submiting with lum buffer when configured to not use one! Set useLum to true before init()!");
		BGFXH_CHECK(inited, "using a bgfxh::bloomEffect without calling init()!");
...
namespace bgfxh
{
  void bloomEffect::submit (bgfx::TextureHandle const & colourBufferIn)
                                                                 {
		BGFXH_CHECK(!useLum, "bgfxh::bloomEffect: submiting without a lum buffer!");
		BGFXH_CHECK(inited, "using a bgfxh::bloomEffect without calling init()!");
