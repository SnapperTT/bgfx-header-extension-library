...
//
// How to use:
// 0. Include! Use #include <bgfxh/bgfxh.h>, #include <bgfxh/tonemappingRenderJob.h> in a relevent file. Be sure to use #define BGFXH_IMPL *once* to generate an implementation somewhere!
// 1. Create an instance of the tonemappingRenderJob object ( bgfxh::tonemappingRenderJob mTonemappingFilter
// 2. Set the user parameters to your liking. Use mTonemappingFilter.setSize(width, height);
// 3. Call tonemappingRenderJob.init() - this will generate framebuffers and uniforms and load the shaders
...
namespace bgfxh
{
  class tonemappingRenderJob : public renderJob
  {
  public:
...
namespace bgfxh
{
  tonemappingRenderJob::tonemappingRenderJob ()
                                { initToZero(); }
}
namespace bgfxh
{
...
namespace bgfxh
{
  void tonemappingRenderJob::setSize (uint32_t const _width, uint32_t const _height)
                                                                     {
		width = _width;
		height = _height; 
		}
...
namespace bgfxh
{
  bgfx::FrameBufferHandle tonemappingRenderJob::getOutputFrameBuffer () const
                                                              { return mOutputFB; }
}
namespace bgfxh
{
  int tonemappingRenderJob::getViewIncrement () const
                                      { return 1; }
}
namespace bgfxh
{
  void tonemappingRenderJob::setExtraComponent (uint32_t slot, bgfx::TextureHandle const texture, float additiveWeight, float multiplicitveWeight)
                                                                                                                                   {
...
namespace bgfxh
{
  void tonemappingRenderJob::initToZero ()
                           {
...
namespace bgfxh
{
  void tonemappingRenderJob::init ()
                     {
		if (!renderToBackbuffer) {
			mOutputFB = bgfx::createFrameBuffer(width, height, framebufferTexFormat, BGFXH_RT_UV_CLAMP);
			bgfx::setName(mOutputFB, "bgfxh::tonemappingRenderJob::mOutputFB");
			}
		...
		#endif
		BGFXH_CHECK(bgfx::isValid(m_tonemappingProgram), "failed to load shader bgfxh::tonemappingRenderJob::m_tonemappingProgram! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
		inited = true;
		}
}
namespace bgfxh
{
  void tonemappingRenderJob::deInit ()
                       {
...
namespace bgfxh
{
  void tonemappingRenderJob::submit (bgfx::TextureHandle const colourBufferIn, bgfx::TextureHandle const lumBufferIn)
                                                                                                      {
