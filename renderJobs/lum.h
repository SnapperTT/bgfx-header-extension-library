...
// 
// How to use:
// 0. Include! Use #include <bgfxh/bgfxh.h>, #include <bgfxh/lumRenderJob.h> in a relevent file. Be sure to use #define BGFXH_IMPL *once* to generate an implementation somewhere!
// 1. Create an instance of the lumRenderJob object ( bgfxh::lumRenderJob mLumFilter; )
...
namespace bgfxh
{
  class lumRenderJob : public renderJob
  {
  public:
...
namespace bgfxh
{
  int lumRenderJob::getViewIncrement () const
                                       {
...
namespace bgfxh
{
  void lumRenderJob::setOffsets2x2Lum (bgfx::UniformHandle _handle, uint32_t const _width, uint32_t const _height, float const _texelHalf)
                                                                                                                                         {
		/*
...
namespace bgfxh
{
  void lumRenderJob::setOffsets4x4Lum (bgfx::UniformHandle _handle, uint32_t const _width, uint32_t const _height, float const _texelHalf)
                                                                                                                                         {
		/*
		* From the BGFX Examples, the following license applies to only this function:
		* Copyright 2011-2018 Branimir Karadzic. All rights reserved.
...
namespace bgfxh
{
  void lumRenderJob::init ()
		...
		BGFXH_CHECK(bgfx::isValid(m_lumProgram), "failed to load shader bgfxh::lumRenderJob::m_lumProgram! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
		BGFXH_CHECK(bgfx::isValid(m_lumAvgProgram), "failed to load shader bgfxh::lumRenderJob::m_lumAvgProgram! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
		BGFXH_CHECK(bgfx::isValid(m_lumAvgOutputProgram), "failed to load shader bgfxh::lumRenderJob::m_lumAvgOutputProgram! Check your bgfxh::shaderSearchPath setting, path, and that the shader type matches the renderer type!");
...
namespace bgfxh
{
  void lumRenderJob::submit (bgfx::TextureHandle const & colourBufferIn, float const frameTime)
                                                                                                     {
		frameTickTock = !frameTickTock;
		BGFXH_CHECK(inited, "using a bgfxh::lumRenderJob without calling init()!");
		...
		bgfxh::lumRenderJob::setOffsets2x2Lum(u_lumOffset, 128, 128, _texelHalf);
		...
		bgfxh::lumRenderJob::setOffsets4x4Lum(u_lumOffset, 128, 128, _texelHalf);
		...
		bgfxh::lumRenderJob::setOffsets4x4Lum(u_lumOffset, 64, 64, _texelHalf);
		...
		bgfxh::lumRenderJob::setOffsets4x4Lum(u_lumOffset, 16, 16, _texelHalf);
		...
		bgfxh::lumRenderJob::setOffsets4x4Lum(u_lumOffset, 4, 4, _texelHalf); // This is a 1x1 buffer
